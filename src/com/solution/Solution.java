...
public class Solution {
	...
	public static void main(String[] args) throws IOException {
		...
		// filteredStudentList.forEach(st -> System.out.println("Id " + st.getId() + "
		// Name " + st.getName()));
		...
		// filteredStudentList2.forEach(st -> System.out.println("Id " + st.getId() + "
		// Name " + st.getName()));
		...
		filteredStudentList2.forEach(
				st -> System.out.println("Id " + st.getId() + " Name  " + st.getName() + " Marks " + st.getMarks()));
	}

	private static List<Student> readFemaleStudentOrderByMarks(List<Student> studentList, int startPage, int pageSize) {

		List<Student> filteredStudentList = studentList.stream().filter(st -> "F".equals(st.getGender()))
				.skip((startPage - 1) * pageSize).limit(pageSize).sorted(Comparator.comparingInt(Student::getMarks))
	...
	private static List<Student> readFemaleStudentOrderByName(List<Student> studentList, int startPage, int pageSize) {
		...
		List<Student> filteredStudentList = studentList.stream().filter(st -> "F".equals(st.getGender()))
				.skip((startPage - 1) * pageSize).limit(pageSize).sorted(Comparator.comparing(Student::getName))
				.collect(Collectors.toList());
	...
	private static List<Student> readFemaleStudentByPage(List<Student> studentList, int startPage, int pageSize) {
		...
		List<Student> filteredStudentList = studentList.stream().filter(st -> "F".equals(st.getGender()))
				.skip((startPage - 1) * pageSize).limit(pageSize).collect(Collectors.toList());
