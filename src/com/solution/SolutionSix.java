...
public class SolutionSix {
	...
	public static void main(String[] args) throws IOException {
		...
		filteredStudent6.forEach(st -> System.out.println("Id " + st.getId() + " Name " + st.getName() + " Gender "
				+ st.getGender() + " Age " + st.getAge() + " Marks " + st.getMarks()));

		/*
		 * for(Student student : filteredStudent6) { System.out.println("Id " +
	...
	private static List<Student> findAllStudent(List<Class> classList, List<Student> studentList,
		...
		List<Integer> classId = classList.stream().filter(cls -> cls.getName().equals(x)).map(cls -> cls.getId())
				.collect(Collectors.toList());

		return studentList.stream().filter(s -> classId.contains(s.getClassId())).collect(Collectors.toList());

		/*
		 * for(Class classT : classList) { if(classT.getName().equals(x)) { int id =
		 * classT.getId();
		 * 
		 * List<Student> studentNew = studentList.stream().filter(s -> s.getClassId() ==
